...
CREATE TABLE ForumPosts (
    ...
    CONSTRAINT FK_ForumPost_User FOREIGN KEY (UserId) REFERENCES Users(UserId)
);
...
CREATE TABLE ForumComments (
    ...
    Content NVARCHAR(MAX) NOT NULL,
    CreatedAt DATETIME DEFAULT GETDATE(),
    CONSTRAINT FK_ForumComment_Post FOREIGN KEY (PostId) REFERENCES ForumPosts(PostId) ON DELETE CASCADE,
...
CREATE TABLE ForumLikes (
    ...
    UserId INT NOT NULL,
    CreatedAt DATETIME DEFAULT GETDATE(),
    CONSTRAINT FK_ForumLike_Post FOREIGN KEY (PostId) REFERENCES ForumPosts(PostId) ON DELETE CASCADE,
    CONSTRAINT FK_ForumLike_User FOREIGN KEY (UserId) REFERENCES Users(UserId),
    CONSTRAINT UQ_ForumLike UNIQUE (UserId, PostId)
);

-- Forum Seed Data
INSERT INTO ForumPosts (UserId, Content, ImageUrl, Category, CreatedAt, LikeCount, CommentCount)
VALUES 
(2, N'Just secured this beautiful studio in Son Tra! The verification process through Staytrust made the legal paperwork so much easier. #DaNangLiving #RentalTips', 'https://raw.githubusercontent.com/ArcherFMY/SD-T2I-360PanoImage/main/data/a-living-room.png', 'RentalTips', DATEADD(HOUR, -2, GETDATE()), 124, 18),
(3, N'Does anyone have tips for finding reliable roommates in Da Nang? I am moving there next month.', NULL, 'DaNangLiving', DATEADD(HOUR, -5, GETDATE()), 45, 12),
(5, N'The new rental regulations in 2024 are quite interesting. Make sure to read up on the changes to deposit laws.', NULL, 'LegalAdvice', DATEADD(DAY, -1, GETDATE()), 89, 7),
(6, N'Just posted a new listing in Hai Ba Trung. It has a beautiful garden and is perfect for a small family!', 'https://images.unsplash.com/photo-1568605114967-8130f3a36994?q=80&w=2070&auto=format&fit=crop', 'VerifiedNews', DATEADD(DAY, -2, GETDATE()), 32, 5);

INSERT INTO ForumComments (PostId, UserId, Content, CreatedAt)
VALUES 
(1, 5, N'That studio looks amazing! I might need to check out Staytrust for my next move.', DATEADD(HOUR, -1, GETDATE())),
